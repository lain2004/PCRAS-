...
A MATLAB-based graphical tool for optimizing **PCRAS** (Progressive Mixing-Roughness-Information with **CRA**ter effect correction) model parameters for GDMS (Glow Discharge Mass Spectrometry) depth profiling of multilayer thin films. Integrates the Starfish Optimization Algorithm (SFOA) and Particle Swarm Optimization (PSO) to fit experimental depth-concentration data.
...
The PCRAS model deconvolves these effects by forward-modeling the measured signal from an assumed layer structure and optimizing the model parameters to match experiment.
...
├── PCRAS_func.m             # PMRI-CRA forward model
